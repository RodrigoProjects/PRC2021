<template>
  <div class="d-flex flex-column align-center justify-center mx-auto">
    <div class="text-h2 font-weight medium">Períodos</div>
    <v-progress-linear
      class="mt-6"
      style="width: 70%"
      v-if="loading"
    ...
    <div v-if="grupos && !loading" class="mt-5" style="width:70%">
      <v-expansion-panels>
        <v-expansion-panel v-for="el in grupos" :key="el">
          <v-expansion-panel-header> Período {{ el.charAt(0).toUpperCase() + el.slice(1) }}</v-expansion-panel-header>
          <v-expansion-panel-content>
            <PeriodElements :periodo="el" />
          </v-expansion-panel-content>
        </v-expansion-panel>
      </v-expansion-panels>
    </div>
...
import PeriodElements from "@/components/PeriodElements"
...
export default {
  ...
  created() {
    fetchOntobud(`select ?s where { ?s rdf:type :Period } `)
      .then((res) => {
